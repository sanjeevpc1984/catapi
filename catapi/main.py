...
from catapi import config
from catapi.error_handler import exception_handler, validation_exception_handler
from catapi.events.event_handlers import EVENT_HANDLERS
from catapi.exceptions import CatapiError
from catapi.libs import log_sanitizer
from catapi.views import cat_view, status_view
...
def add_exception_handlers(app: FastAPI) -> None:
    ...
    app.add_exception_handler(CatapiError, exception_handler)
...
app = FastAPI(
    title="Catapi", description="Lionbridge AI Cat Management Service", version=config.VERSION
)
init_sentry(app)
init_apm(app)
include_routers(app)
