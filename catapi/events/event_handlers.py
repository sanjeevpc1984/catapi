...
def handle_cat_created(data: dto.JSON) -> None:
    ...
    asyncio.run(_handle_cat_created(data))


async def _handle_cat_created(data: dto.JSON) -> None:
    cat_id = data.get("cat_id")
    cat_filter = dto.CatFilter(cat_id=cat_id)
    cat_partial_update = dto.PartialUpdateCat()
    await cat_domain.partial_update_cat(cat_filter, cat_partial_update)
