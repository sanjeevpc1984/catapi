...
autoformat() {
  ...
  echo '---- format (isort) ----'
  poetry run isort catapi tests $ISORT_FLAG
}

check_style() {
  echo '---- style ----'
  poetry run flake8 catapi tests
}

check_type() {
  echo '---- type ----'
  poetry run mypy catapi tests
}

check_unit() {
  # default to coverage mode
  PYTEST_FLAGS='--cov=catapi --ignore=catapi --cov-report=term-missing --cov-report=xml'
